...
- `infra/docker/grafana.Dockerfile` — `grafana/grafana-oss:11.3.0` (pinned, not `:latest`) with
  ...
  - `dashboards/infra-dashboard.json` — one dashboard (`chatapp-infra`), three row groups:
    ...
    - **ElastiCache** (CPUUtilization, CurrConnections) and **Container Insights**
      (CpuUtilized/MemoryUtilized under the `ECS/ContainerInsights` namespace) — both intentionally
      showed "No data" while this session ran concurrently with session 09, per the relaxed
      prerequisite. **Post-merge correction**: the ElastiCache panels' placeholder dimension
      (`CacheClusterId=chatapp-redis-001`, a guess made before session 09's cluster existed) didn't
      match the real cluster id session 09 actually created (`chatapp-redis`, no `-001` suffix —
      confirmed via `aws elasticache describe-cache-clusters`). Fixed in
      `infra/docker/grafana/provisioning/dashboards/infra-dashboard.json`, rebuilt and redeployed
      as `chatapp-grafana:manual-2` (task def revision 2) during the session 09/10 merge, and
      reverified live: `POST /grafana/api/ds/query` for `AWS/ElastiCache` `CPUUtilization` with the
      corrected dimension returned 6 real datapoints. Worth remembering: dashboard dimensions that
      reference another concurrently-running session's not-yet-created resources are a real risk
      of this kind of parallel session — always worth a dimension spot-check after both land.
